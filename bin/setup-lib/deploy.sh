...
do_npm() {
    ...
    for sub in "" bots/liquidity bots/lending bots/settings core/nexus core/premium core/wallet-monitor; do
        ...
        # --prefer-offline (2026-08-07): do_deploy() löscht app/ bei jedem Update
        # komplett, node_modules kann also nie über ein Update hinweg wiederverwendet
        # werden — ein reiner "Version schon installiert?"-Check liefe ins Leere.
        # npms eigener Paket-Cache liegt aber im Home-Verzeichnis von $INSTALL_USER,
        # NICHT unter app/, und übersteht das rm -rf also unberührt. --prefer-offline
        # lässt npm die Paket-Metadaten aus diesem Cache nehmen statt sie bei jedem
        # Lauf neu über die Registry abzufragen — spart Netzwerk-Rundlaufzeiten, ohne
        # das "app/ ist nach jedem Deploy garantiert sauber"-Prinzip anzutasten.
        sudo -u "$INSTALL_USER" bash -c "cd '$dir' && npm install --silent --prefer-offline" \
