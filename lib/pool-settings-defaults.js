 ...
 * überlebt jeden Kapitalabzug (Trailing Stop, TVL-Schutz, manueller Withdraw). Zurückgesetzt wird ausschließlich, was
 ...
 * Claim-Betrag (autoCompound) und – am
...
export const POOL_SETTINGS_DEFAULTS = {
    ...
    //                       Braucht Premium (die Empfehlung wird zentral gerechnet und
    //                       geliefert) und eine belastbare Datenlage. Liefert der Advisor
    //                       nichts, gelten die unten gesetzten Werte; sind auch die leer,
    //                       greift TS_AUTO_FALLBACK (1,75 % / 0,75 %) aus lib/trailing-stop.js.
    //
    // 🔒 Die Schwellen müssen mit FALLBACK_TS_CONFIG in bots/liquidity/lib/trailing-stop.js
    // übereinstimmen. Fallen sie auseinander, zeigt ForgeSettings für einen Pool ohne
    // Sektion andere Werte an, als der Bot anwendet — genau der Fall vom 2026-08-01
    // (PUMP/SOL lief ohne Stop, während das UI „aktiv, 10 %" auswies).
    //
    // 1,75 % / 0,75 % seit 2026-08-30 abends (LIQ#0351), davor kurz 0,8/0,6 (verworfener
    // Backtest), ursprünglich 10 % ohne zweite Stufe. Grundlage ist die Zitter-Messung des
    // Trailing-Stop-Advisors; Begründung und Grenzen: siehe
    // `Liquidity Bot/trailing-stop-advisor.md`.
    trailingStop: {
        enabled:         true,
        thresholdPct:    1.75,
        thresholdPct2:   0.75,   // zweite Stufe; null = nur einstufig
        auto:            false,  // Advisor-Empfehlung übernehmen; Default aus
        minimumValueUsd: null,   // Pool-Mindestwert in USDC; null = deaktiviert
        autoSwapToUSDC:  true,
        sendTo:          '',
        // Sperrfrist für automatisches Cleanup-Reinvest nach TS-Exit. 6 h seit 2026-09-03
        // (LIQ#0359), vorher 1 h: Der Cleanup läuft stündlich, 1 h blockierte in 178
        // Episoden keinen einzigen Wiedereinstieg; 6 h hätte 63 Wiedereinstiege mit in
        // Summe negativem Ergebnis verhindert. Begründung und Zahlen:
    ...
    // Max Investment: harte Obergrenze für Kapital, das per Cleanup-Reinvest oder
    // manueller Einzahlung in diesen Pool fließen darf (Risikobegrenzung bei
    // volatilen Pools). amountUsdc = null → unbegrenzt (Default).
    //   Vergleichsgröße ist positions.capital_usdc (gebuchtes Einzahlungskapital),
    //   NICHT der Markt-Wert (lp_value_usd) — sonst würde ein Kursrückgang wieder
    //   Headroom freigeben und in der Baisse nachkaufen, das Gegenteil des Zwecks.
    //   Wird nur beim Hineinlegen geprüft: Autocompounding (bot.js) und ein bereits
    //   über dem Cap liegender Bestand bleiben unangetastet — kein automatischer
    //   Teilausstieg, der Cap blockiert ausschließlich zukünftige Invests.
    maxInvestment: {
        enabled:    false,
        amountUsdc: null,
    },
    // TVL-Schutz: Pool-Exit wenn der Pool-TVL unter eine Schwelle fällt.
    //   Beim Unterschreiten der Schwelle wird withdrawPct % des Kapitals abgezogen.
    //   thresholdUsd === null → noch nie konfiguriert; UI füllt mit der pools.json-
    //   Exit-Schwelle (tvlExitThreshold) vor.
    //   tvlAtActivation wird vom Liquidity-Bot beim ersten Deposit gesetzt (read-only fürs UI).
    //
    // Default (seit 2026-08-15): 100 % Voll-Exit bei einer Schwelle. Eine frühere zweite,
    // tiefere Stufe (level2) wurde nie konfiguriert und ist entfernt (LIQ#0324).
    tvlProtection: {
        level1: {
            enabled:      true,
            thresholdUsd: null,
        ...
        swapToUsdc:      true,   // in USDC tauschen
        sendTo:          '',     // Empfänger (leer = Wallet)
    ...
    // Entfallen 2026-08-15: die Sektion `ranking` (Ranking-Exit), 2026-09-23 die Sektion
    // `scoreLimit` (Score-Limit-Exit, LIQ#000925). Beide Features wurden ausgebaut —
    // bestehende DB-Einträge behalten den Schlüssel, er wird von loadSettings() und
    // diffFromDefaults() schlicht ignoriert.
};
...
/**
 * Vergleicht zwei Settings-Objekte (vor/nach einem Speichervorgang) und liefert jedes
 * Blattfeld, dessen Wert sich geändert hat — Grundlage für `settings_history`
 * (bots/settings/routes/pools.js), damit sich z.B. „wann wurde thresholdPct2 von 0,75 auf
 * 0,5 geändert" nachträglich beantworten lässt, ohne auf einen zufällig zeitnahen
 * ts_executions-Eintrag angewiesen zu sein (Lücke, belegt 2026-08-23).
 *
 * Anders als `diffFromDefaults()` läuft dies über die UNION der Schlüssel aus beiden
 * Objekten (nicht nur über POOL_SETTINGS_DEFAULTS) — funktioniert dadurch sowohl für
 * pool_settings als auch für die schlankere Struktur von pool_type_settings.
 *
 * '' und null gelten als bedeutungsgleich „nicht gesetzt" (gleiche Konvention wie
 * diffFromDefaults), damit ein Leerstring-Roundtrip nicht als Änderung durchgeht.
 *
 * @param {Object} before
 * @param {Object} after
 * @param {{ skipPaths?: string[] }} [opts]  Pfade, die nie als Änderung zählen sollen
 *   (z.B. POOL_SESSION_FIELDS — Bot-Zustand, keine Nutzer-Einstellung).
 * @returns {Array<{path: string, oldValue: *, newValue: *}>}
 */
export function diffChangedFields(before, after, { skipPaths = [] } = {}) {
    const out = [];
    const isPlainObject = v => v !== null && typeof v === 'object' && !Array.isArray(v);
    const norm = v => (v === '' || v === null || v === undefined ? null : v);
    const walk = (o, n, prefix) => {
        ...
        for (const key of keys) {
            ...
            if (isPlainObject(ov) || isPlainObject(nv)) {
                walk(isPlainObject(ov) ? ov : {}, isPlainObject(nv) ? nv : {}, path);
                continue;
            }
            if (norm(ov) !== norm(nv)) out.push({ path, oldValue: ov ?? null, newValue: nv ?? null });
        }
    };
    walk(before ?? {}, after ?? {}, '');
    return out;
