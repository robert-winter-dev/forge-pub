    ...
    liquidity: {
        ...
        earningsOutCurve(db, scopeId, fromMs, toMs) {
            ...
            const baseClaimed = db.prepare(
                `SELECT COALESCE(SUM(usd_value),0) AS t FROM transactions
                  WHERE ${where}type='claim' AND created_at < ?`).get(...baseArgs).t;
            ...
            const events = db.prepare(`
                SELECT created_at AS t, type, usd_value AS v FROM transactions
                 WHERE ${where}type IN ('claim','reinvest') AND usd_value IS NOT NULL
...
/**
 * Historischer Höchststand des PnL im Zeitraum [fromMs, jetzt] für EINEN Scope
 * (z.B. Anteil-Tooltip: seit wann stand der Pool am weitesten im Plus, und wie
 * hoch war das). Tastet dieselbe Kurve ab wie pnlForPeriod — Snapshot-Punkte
 * plus etwaige Claim/Reinvest-Sprünge (earningsOutCurve) — sucht darauf das
 * Maximum, statt nur den End-minus-Start-Wert zu bilden.
 * @returns {{pnlUsd:number, atMs:number}|null} null wenn der Scope keine Daten hat.
 */
export function pnlPeakForPeriod(db, { flavor, scope, fromMs, toMs = null }) {
    const a   = _adapter(flavor);
    const now = Date.now();
    const end = toMs ?? now;
    const byScope = _buildCurves(db, flavor, now, 0);
    ...
    const lastPoint = { t: end, pnl: toMs == null ? c.latestPnl : _curveAt(c.history, end) };
    if (!points.length || points[points.length - 1].t < end) points.push(lastPoint);

    let best = null;
    for (const p of points) {
        const pnlUsd = round2((p.pnl - startPnl) + eoAt(p.t));
        if (best == null || pnlUsd > best.pnlUsd) best = { pnlUsd, atMs: p.t };
    }
    return best;
}
