...
/**
 * Gibt es zu diesem Key überhaupt einen Text — hier oder in der Referenzsprache?
 *
 * Für Aufrufer, die einen fehlenden Key ERWARTEN und selbst eine bessere Antwort haben.
 * Der Fall gehört `lib/notify-render.js`: Eine gespeicherte Meldung überlebt den
 * Katalogeintrag, mit dem sie geschrieben wurde (Rückbau, Umbenennung). `t()` würde dort
 * den rohen Key ausliefern und pro Aufruf warnen — obwohl der fertige deutsche Text in
 * der Zeile daneben steht.
 *
 * Nicht als allgemeine Vorprüfung vor `t()` verwenden: Ein Tippfehler in NEUEM Code soll
 * weiterhin über die Warnung in `t()` auffallen, nicht stillschweigend verschwinden.
 *
 * @param {string} key
 * @param {{lang?: string, scope?: 'backend'|'frontend'}} [opts]
 * @returns {boolean}
 */
export function hasKey(key, opts = {}) {
    const lang  = opts.lang ?? getLang();
    const scope = opts.scope ?? 'backend';
    const here  = loadCatalog(scope, lang)[key];
    if (typeof here === 'string' && here !== '') return true;
    if (lang === DEFAULT_LANG) return false;
    const ref = loadCatalog(scope, DEFAULT_LANG)[key];
