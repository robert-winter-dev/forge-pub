...
//
// Fünf Konventionen, die den Katalog schlank halten:
//
...
//
//   4. **Zahlen kommen roh, formatiert wird hier** (LIQ#000885). Ein Parameter
//      `{ n: 22.76, d: 2, sign: true }` wird erst beim Anzeigen in der Zielsprache
//      gesetzt: deutsch `+22,76`, englisch `+22.76`, negativ immer mit echtem
//      Minuszeichen (−, U+2212) statt Bindestrich. Vorher übergaben die Bots
//      `toFixed(2)`-Strings — die standen dann englisch formatiert in deutschen
//      Meldungen, und eine Sprachumschaltung hätte sie nie erreicht (derselbe
//      Grund, aus dem der Text selbst erst hier entsteht). `d` = Nachkommastellen
//      (Vorgabe 2), `sign` = Pluszeichen bei positiven Werten.
//
//   5. **Zeitpunkte kommen als `{ t: <ms> }`, nie als fertiger String** (LIQ#000886).
//      Gerendert mit derselben `Intl.DateTimeFormat`-Regel wie die Kopfzeile
//      (`numLocale(lang)`, Zeitzone `FORGE_TZ`) — deutsch `22.09.2026, 11:15`,
//      englisch `09/22/2026, 11:15 AM`. Vorher stand `toLocaleString('de-DE')`
//      fest im Bot-Code, auch in englischen Meldungen.
//
...
/**
 * Zahl in der Zielsprache (Konvention 4). Exportiert, damit Aufrufer, die einen Wert
 * in einen Freitext einbauen müssen, dieselbe Regel nehmen statt einer eigenen.
 *
 * @param {number} n
 * @param {string} [lang]
 * @param {{d?: number, sign?: boolean}} [opts]
 * @returns {string}  z.B. „−22,76" (de) / „−22.76" (en); nicht-numerisch → „n/a"
 */
export function formatNumber(n, lang = getLang(), { d = 2, sign = false } = {}) {
    const v = Number(n);
    if (!Number.isFinite(v)) return 'n/a';
    const text = new Intl.NumberFormat(numLocale(lang), {
...
/**
 * Zeitpunkt in der Zielsprache (Konvention 5), gleiche Regel wie header().
 *
 * @param {number} ms
 * @param {string} [lang]
 * @returns {string}  z.B. „22.09.2026, 11:15" (de) / „09/22/2026, 11:15 AM" (en)
 */
export function formatTimestamp(ms, lang = getLang()) {
    const v = Number(ms);
    if (!Number.isFinite(v)) return 'n/a';
...
/** Löst `{ k, p }`-Verweise (Konvention 2), `{ n }`-Zahlen (Konvention 4) und
 *  `{ t }`-Zeitpunkte (Konvention 5) auf. */
function resolveParams(params, lang) {
    ...
    for (const [key, value] of Object.entries(params)) {
        ...
        } else if (value && typeof value === 'object' && typeof value.n === 'number') {
            out[key] = formatNumber(value.n, lang, { d: value.d, sign: value.sign });
        } else if (value && typeof value === 'object' && typeof value.t === 'number') {
            out[key] = formatTimestamp(value.t, lang);
        } else if (value != null) {
            out[key] = value;
        }
...
export default { renderNotification, notificationText, formatNumber };
