...
export function subscribeDirectMessages(pool, relays, myIdentity, onMessage) {
    ...
    // 🔴 Fund 2026-08-08, zurückgenommen: ein früherer Versuch, hier einen `since`-Cursor
    // (höchstes je gesehenes `created_at`) an den Watchdog-Resubscribe mitzugeben, hat die
    // Premium-Blob-Auslieferung auf forge-pub1/pub2 komplett stillgelegt. Grund: NIP-17
    // Gift-Wraps (kind 1059 – GENAU das, was hier gefiltert wird) haben laut Spezifikation
    // ABSICHTLICH randomisierte `created_at`-Werte (Anti-Korrelations-Schutz) – spätere,
    // legitime Nachrichten können einen KLEINEREN Zufalls-Offset haben als eine frühere und
    // wurden dadurch vom `since`-Filter dauerhaft verschluckt. `since` ist auf dieser Ebene
    // (Gift-Wrap-Envelope) grundsätzlich nicht sicher nutzbar. Der eigentliche Bug (Reissue-
    // Schleife durch Backlog-Replay nach Rubriken-Cap-Pruning) ist stattdessen serverseitig
    // gefixt, siehe markActivationEventProcessed() in server.js/messages-db.js – exakt das
    // Muster, das schon 2026-08-03 für premium-blob-sealed denselben Bug gelöst hat.
