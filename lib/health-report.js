 ...
 *                 detail:?string, mem_bytes:?number, restarts:?number, uptime_sec:?number}>} rows
...
export function buildHealthReport(rows, header) {
    ...
    for (const [serviceId, svcRowsUnsorted] of byService) {
        ...
        services.push({
            ...
            detailCode:    classifyDetailCode(last.status, last.detail),
        });
...
function fmtWindow(windowFrom, windowTo) {
    const fmt = iso => new Date(iso).toLocaleString('de-DE', {
        day: '2-digit', month: '2-digit', year: 'numeric',
        hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Berlin',
    });
    return `${fmt(windowFrom)} – ${new Date(windowTo).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Berlin' })} Uhr`;
}
 ...
 */
export function formatHealthReportText(report) {
    const lines = [];
    lines.push(`Stündlicher Health Monitor Report`);
    lines.push(`Zeitfenster: ${fmtWindow(report.windowFrom, report.windowTo)}`);
    lines.push(`Version: ${report.version} · OS: ${report.os}`);
    lines.push('');
    lines.push('Dienste (Status je Prüfung in diesem Fenster, Median-/Max-Latenz):');
    for (const s of report.services) {
        const counts = `ok=${s.statusCounts.ok} warn=${s.statusCounts.warn} crit=${s.statusCounts.crit} unknown=${s.statusCounts.unknown}`;
        const latency = s.latencyMedian != null
            ? ` · Latenz ${Math.round(s.latencyMedian)}ms (max ${Math.round(s.latencyMax)}ms)`
            : '';
        lines.push(`- ${s.serviceId}: ${counts}${latency} · ${s.detailCode}`);
    }
    lines.push('');
    lines.push('Prozesse (Speicher, Laufzeit, Neustarts):');
    for (const p of report.processes) {
        lines.push(`- ${p.serviceId}: ${fmtMemBytes(p.memMedian)} (max ${fmtMemBytes(p.memMax)}) · Laufzeit ${fmtUptimeSec(p.uptimeSec)} · Neustarts ${p.restarts ?? 0}`);
    }
    return lines.join('\n');
}
