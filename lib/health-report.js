...
import { readQuotaState, QUOTA_SERVICE_ID, QUOTA_CODE_STATUS } from './quota-state.js';
...
export const DETAIL_CODES = [
    ...
    // Virtueller Dienst `quota-helius` (CORE#000810): nur der Zustand, nie eine Zahl.
    'quota_ok', 'quota_near', 'quota_exhausted', 'quota_unknown',
];
 ...
 * @param {{version:string, versionCode:number, os:string, windowFrom:string, windowTo:string,
 *          quotaCode?: string}} header   quotaCode → virtueller Dienst `quota-helius` (CORE#000810)
 */
export function buildHealthReport(rows, header) {
    ...
    // Kontingent-Zustand als virtueller Dienst: ein Zustandsmesswert je Stunde, in die
    // Spalte gezählt, die zum Zustand passt. Dadurch greifen F1/F2 im Advisor unverändert.
    if (header.quotaCode && header.quotaCode in QUOTA_CODE_STATUS) {
        const statusCounts = { ok: 0, warn: 0, crit: 0, unknown: 0 };
        statusCounts[QUOTA_CODE_STATUS[header.quotaCode]] = 1;
        services.push({ serviceId: QUOTA_SERVICE_ID, statusCounts,
                        latencyMedian: null, latencyMax: null, detailCode: header.quotaCode });
    }
