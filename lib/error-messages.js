 ...
 *
 * @param {Error|any} err
 * @returns {{reason: string, detail: string, detailMode: 'none'|'inline'|'logref'}}
 *   reason:     kurzer, verständlicher Satz (Deutsch) — Übersetzung wenn bekannt,
 *               sonst eine neutrale Kurzbeschreibung inkl. extrahiertem Fehlercode.
...
const KNOWN_PATTERNS = [
    { code: 'INSUFFICIENT_FUNDS', test: /insufficient funds|0x1\n|0x1\.|0x1"/i,
      reason: 'Nicht genug Guthaben für die Transaktion' },
    { code: 'LIQUIDITY_ZERO', test: /0x177c|LiquidityZero|ZeroLiquidity/i,
      reason: 'Betrag zu gering (Staub-Liquidität)' },
    { code: 'LIQUIDITY_UNDERFLOW', test: /0x177f|LiquidityUnderflow/i,
      reason: 'Positions-Liquidität hat sich zwischenzeitlich geändert (Stale Read)' },
    { code: 'CLOSE_NOT_EMPTY', test: /0x1775|ClosePositionNotEmpty/i,
      reason: 'Position hatte beim Schließen noch Restliquidität (Stale Read)' },
    { code: 'SLIPPAGE', test: /0x1782|TokenMinSubceeded/i,
      reason: 'Preis hat sich zu stark bewegt (Slippage)' },
    { code: 'SLIPPAGE', test: /0x17b5|slippage/i,
      reason: 'Preis hat sich zu stark bewegt (Slippage)' },
    { code: 'ROUTING', test: /0x1771|InvalidStartTick/i,
      reason: 'Routing-Fehler beim Swap (Marktpreis hat sich während der Berechnung bewegt)' },
    { code: 'NETWORK_TIMEOUT', test: /Blockhash not found|BlockheightExceeded/i,
      reason: 'Netzwerk-Timeout (Block abgelaufen)' },
];
...
function normalizeConfirmTxError(raw) {
    try {
        ...
        const custom = parsed?.InstructionError?.[1]?.Custom;
        if (typeof custom === 'number') {
            return `${raw} (custom program error: 0x${custom.toString(16)})`;
        }
    } catch (_) { /* kein JSON / nicht diese Form — unverändert lassen */ }
    return raw;
}
...
export function describeError(err) {
    ...
    if (known) {
        return { reason: known.reason, detail, detailMode: 'none' };
    }

    const code = extractProgramErrorCode(raw);
    const reason = code
        ? `Unbekannter Programmfehler (${code})`
        : 'Unerwarteter Fehler';
    const detailMode = isComplexRaw(raw) ? 'logref' : 'inline';
    return { reason, detail, detailMode };
}
